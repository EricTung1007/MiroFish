...
Zep 的 node/edge 列表接口使用 UUID cursor 分頁，
本模塊封裝自動翻頁邏輯（含單頁重試），對調用方透明地返回完整列表。
"""
...
def fetch_all_nodes(
    ...
    """分頁獲取圖譜節點，最多返回 max_items 條（默認 2000）。每頁請求自帶重試。"""
    all_nodes: list[Any] = []
