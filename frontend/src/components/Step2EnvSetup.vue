...
// State
const phase = ref(0) // 0: 初始化, 1: 生成人設, 2: 生成配置, 3: 完成
...
// 模擬輪數配置
const useCustomRounds = ref(false) // 默認使用自動配置輪數
const customMaxRounds = ref(40)   // 默認推薦40輪

// Watch stage to update phase
watch(currentStage, (newStage) => {
  if (newStage === '生成Agent人設' || newStage === 'generating_profiles') {
    phase.value = 1
  } else if (newStage === '生成模擬配置' || newStage === 'generating_config') {
    phase.value = 2
    // 進入配置生成階段，開始輪詢配置
    if (!configTimer) {
  ...
  } else if (newStage === '準備模擬腳本' || newStage === 'copying_scripts') {
    phase.value = 2 // 仍屬於配置階段
  }
})

// 從配置中計算自動生成的輪數（不使用硬編碼默認值）
const autoGeneratedRounds = computed(() => {
  if (!simulationConfig.value?.time_config) {
    return null // 配置未生成時返回 null
  }
  const totalHours = simulationConfig.value.time_config.total_simulation_hours
  const minutesPerRound = simulationConfig.value.time_config.minutes_per_round
  if (!totalHours || !minutesPerRound) {
    return null // 配置數據不完整時返回 null
  }
  const calculatedRounds = Math.floor((totalHours * 60) / minutesPerRound)
  // 確保最大輪數不小於40（推薦值），避免滑動條範圍異常
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    console.warn('獲取 Profiles 失敗:', err)
  }
}
