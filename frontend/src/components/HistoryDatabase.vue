...
const selectedProject = ref(null)  // 當前選中的項目（用於彈窗）
let observer = null
let isAnimating = false  // 動畫鎖，防止閃爍
let expandDebounceTimer = null  // 防抖定時器
let pendingState = null  // 記錄待執行的目標狀態
...
const containerStyle = computed(() => {
  ...
  // 計算實際需要的高度：行數 * 卡片高度 + (行數-1) * 間距 + 少量底部間距
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // 更新待執行的目標狀態（無論是否在動畫中都要記錄最新的目標狀態）
        pendingState = shouldExpand
        
        // 清除之前的防抖定時器（新的滾動意圖會覆蓋舊的）
        if (expandDebounceTimer) {
          clearTimeout(expandDebounceTimer)
          expandDebounceTimer = null
        }
        
        // 如果正在動畫中，只記錄狀態，等動畫結束後處理
        ...
        // 使用防抖延遲狀態切換，防止快速閃爍
        // 展開時延遲較短(50ms)，收起時延遲較長(200ms)以增加穩定性
    ...
    },
    {
      ...
      // 調整 rootMargin，視口底部向上收縮，需要滾動更多才觸發展開
...
.grid-pattern {
  ...
  /* 從左上角開始定位，高度變化時只在底部擴展，不影響已有網格位置 */
...
.cards-container {
  ...
  /* min-height 由 JS 動態計算，根據卡片數量自適應 */
}
...
/* 不同功能的顏色 */
.status-icon:nth-child(1).available { color: #3B82F6; } /* 圖譜構建 - 藍色 */
.status-icon:nth-child(2).available { color: #F59E0B; } /* 環境搭建 - 橙色 */
.status-icon:nth-child(3).available { color: #10B981; } /* 分析報告 - 綠色 */
...
/* 進度狀態顏色 */
.card-progress.completed { color: #10B981; }    /* 已完成 - 綠色 */
.card-progress.in-progress { color: #F59E0B; }  /* 進行中 - 橙色 */
