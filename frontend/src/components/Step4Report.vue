            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (內容生成完成，但整個章節可能還沒完成) -->
...
const toolConfig = {
  'insight_forge': {
    name: 'Deep Insight',
    color: 'purple',
    icon: 'lightbulb' // 燈泡圖標 - 代表洞察
  },
  'panorama_search': {
    name: 'Panorama Search',
    color: 'blue',
    icon: 'globe' // 地球圖標 - 代表全景搜索
  },
  'interview_agents': {
    name: 'Agent Interview',
    color: 'green',
    icon: 'users' // 用戶圖標 - 代表對話
  },
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
  ...
  'get_graph_statistics': {
    name: 'Graph Stats',
    color: 'cyan',
    icon: 'chart' // 圖表圖標 - 代表統計
  },
  'get_entities_by_type': {
    name: 'Entity Query',
    color: 'pink',
    icon: 'database' // 數據庫圖標 - 代表實體
  }
}
...
const parseInsightForge = (text) => {
  ...
  try {
    // 提取分析問題
    const queryMatch = text.match(/分析問題:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    // 提取預測場景
    const reqMatch = text.match(/預測場景:\s*(.+?)(?:\n|$)/)
    if (reqMatch) result.simulationRequirement = reqMatch[1].trim()
    
    // 提取統計數據 - 匹配"相關預測事實: X條"格式
    const factMatch = text.match(/相關預測事實:\s*(\d+)/)
    const entityMatch = text.match(/涉及實體:\s*(\d+)/)
    const relMatch = text.match(/關係鏈:\s*(\d+)/)
    if (factMatch) result.stats.facts = parseInt(factMatch[1])
    if (entityMatch) result.stats.entities = parseInt(entityMatch[1])
    if (relMatch) result.stats.relationships = parseInt(relMatch[1])
    
    // 提取子問題 - 完整提取，不限制數量
    const subQSection = text.match(/### 分析的子問題\n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
    ...
    // 提取關鍵事實 - 完整提取，不限制數量
    const factsSection = text.match(/### 【關鍵事實】[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (factsSection) {
    ...
    // 提取核心實體 - 完整提取，包含摘要和相關事實數
    const entitySection = text.match(/### 【核心實體】\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
      ...
      result.entities = entityBlocks.map(block => {
        ...
        const relatedMatch = block.match(/相關事實:\s*(\d+)/)
        return {
    ...
    // 提取關係鏈 - 完整提取，不限制數量
    const relSection = text.match(/### 【關係鏈】\n([\s\S]*?)(?=\n###|$)/)
    if (relSection) {
...
const parsePanorama = (text) => {
  ...
  try {
    // 提取查詢
    const queryMatch = text.match(/查詢:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    // 提取統計數據
    const nodesMatch = text.match(/總節點數:\s*(\d+)/)
    const edgesMatch = text.match(/總邊數:\s*(\d+)/)
    const activeMatch = text.match(/當前有效事實:\s*(\d+)/)
    const histMatch = text.match(/歷史\/過期事實:\s*(\d+)/)
    ...
    // 提取當前有效事實 - 完整提取，不限制數量
    const activeSection = text.match(/### 【當前有效事實】[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (activeSection) {
    ...
    // 提取歷史/過期事實 - 完整提取，不限制數量
    const histSection = text.match(/### 【歷史\/過期事實】[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (histSection) {
    ...
    // 提取涉及實體 - 完整提取，不限制數量
    const entitySection = text.match(/### 【涉及實體】\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
...
const parseInterview = (text) => {
  ...
  try {
    // 提取採訪主題
    const topicMatch = text.match(/\*\*採訪主題:\*\*\s*(.+?)(?:\n|$)/)
    if (topicMatch) result.topic = topicMatch[1].trim()
    
    // 提取採訪人數（如 "5 / 9 位模擬Agent"）
    const countMatch = text.match(/\*\*採訪人數:\*\*\s*(\d+)\s*\/\s*(\d+)/)
    if (countMatch) {
    ...
    // 提取採訪對象選擇理由
    const reasonMatch = text.match(/### 採訪對象選擇理由\n([\s\S]*?)(?=\n---\n|\n### 採訪實錄)/)
    if (reasonMatch) {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // 格式1: 數字. **名字（index=X）**：理由
        // 例如: 1. **校友_345（index=1）**：作為武大校友...
        ...
        // 格式2: - 選擇名字（index X）：理由
        // 例如: - 選擇家長_601（index 0）：作為家長群體代表...
        if (!headerMatch) {
          headerMatch = line.match(/^-\s*選擇([^（(]+)(?:[（(]index\s*=?\s*\d+[)）])?[：:]\s*(.*)/)
          if (headerMatch) {
            name = headerMatch[1].trim()
            reasonStart = headerMatch[2]
          }
        ...
        } else if (currentName && line.trim() && !line.match(/^未選|^綜上|^最終選擇/)) {
          // 理由的續行（排除結尾總結段落）
    ...
    // 提取每個採訪記錄
    const interviewBlocks = text.split(/#### 採訪 #\d+:/).slice(1)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // 提取簡介
      const bioMatch = block.match(/_簡介:\s*([\s\S]*?)_\n/)
      if (bioMatch) {
      ...
      const answerMatch = block.match(/\*\*A:\*\*\s*([\s\S]*?)(?=\*\*關鍵引言|$)/)
      if (answerMatch) {
        const answerText = answerMatch[1].trim()
        
        // 分離Twitter和Reddit回答
        const twitterMatch = answerText.match(/【Twitter平臺回答】\n?([\s\S]*?)(?=【Reddit平臺回答】|$)/)
        const redditMatch = answerText.match(/【Reddit平臺回答】\n?([\s\S]*?)$/)
        ...
        // 平臺回退邏輯（兼容舊格式：只有一個平臺標記的情況）
        if (!twitterMatch && redditMatch) {
          // 只有 Reddit 回答，僅在非佔位文本時複製為默認顯示
          if (interview.redditAnswer && interview.redditAnswer !== '（該平臺未獲得回覆）') {
            interview.twitterAnswer = interview.redditAnswer
          }
        } else if (twitterMatch && !redditMatch) {
          if (interview.twitterAnswer && interview.twitterAnswer !== '（該平臺未獲得回覆）') {
      ...
      // 提取關鍵引言（兼容多種引號格式）
      const quotesMatch = block.match(/\*\*關鍵引言:\*\*\n([\s\S]*?)(?=\n---|\n####|$)/)
      if (quotesMatch) {
        const quotesText = quotesMatch[1]
        // 優先匹配 > "text" 格式
        let quoteMatches = quotesText.match(/> "([^"]+)"/g)
        // 回退：匹配 > "text" 或 > \u201Ctext\u201D（中文引號）
        if (!quoteMatches) {
    ...
    // 提取採訪摘要
    const summaryMatch = text.match(/### 採訪摘要與核心觀點\n([\s\S]*?)$/)
    if (summaryMatch) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    // 提取搜索查詢
    const queryMatch = text.match(/搜索查詢:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    // 提取結果數量
    const countMatch = text.match(/找到\s*(\d+)\s*條/)
    if (countMatch) result.count = parseInt(countMatch[1])
    
    // 提取相關事實 - 完整提取，不限制數量
    const factsSection = text.match(/### 相關事實:\n([\s\S]*)$/)
    if (factsSection) {
      const lines = factsSection[1].split('\n').filter(l => l.match(/^\d+\./))
    ...
    // 嘗試提取邊信息（如果有）
    const edgesSection = text.match(/### 相關邊:\n([\s\S]*?)(?=\n###|$)/)
    if (edgesSection) {
    ...
    // 嘗試提取節點信息（如果有）
    const nodesSection = text.match(/### 相關節點:\n([\s\S]*?)(?=\n###|$)/)
    if (nodesSection) {
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '（該平臺未獲得回覆）' || t === '(該平臺未獲得回覆)' || t === '[無回覆]'
    }
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // 分割失敗：第一個問題返回完整回答，其餘返回空
      return qIdx === 0 ? answer : ''
    }
    
    // 檢查某個問題是否有雙平臺回答（過濾佔位文本）
            ...
            return h('div', { class: 'qa-pair', key: qIdx }, [
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  ...
                  // Expand/Collapse Button（佔位文本不顯示）
...
const activeStep = computed(() => {
  ...
  // 如果沒有 active，返回最後一個 done 的步驟
  const doneSteps = steps.filter(s => s.status === 'done')
  if (doneSteps.length > 0) return doneSteps[doneSteps.length - 1]
  
  // 否則返回第一個步驟
  return steps[0] || { noLabel: '--', title: '等待開始', status: 'todo', meta: '' }
})
...
const renderMarkdown = (content) => {
  ...
  // 清理 <p><br> 緊跟塊級元素的情況（多餘空行導致）
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // 清理連續的 <br> 標籤
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // 清理塊級元素後緊跟的段落開始標籤前的 <br>
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // 修復非連續有序列表的編號：當單項 <ol> 被段落內容隔開時，保持編號遞增
...
const getLogLevelClass = (log) => {
  if (log.includes('ERROR') || log.includes('錯誤')) return 'error'
  if (log.includes('WARNING') || log.includes('警告')) return 'warning'
  // INFO 使用默認顏色，不標記為 success
  return ''
}
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            currentSectionIndex.value = null  // 確保清除 loading 狀態
            emit('update-status', 'completed')
            stopPolling()
            // 滾動邏輯統一在循環結束後的 nextTick 中處理
          }
...
// 提取最終答案內容 - 從 LLM response 中提取章節內容
const extractFinalContent = (response) => {
  if (!response) return null
  
  // 嘗試提取 <final_answer> 標籤內的內容
  ...
  // 嘗試找 最終答案: 後面的內容
  const chineseFinalMatch = response.match(/最終答案[:：]\s*\n*([\s\S]*)$/i)
  if (chineseFinalMatch) {
    return chineseFinalMatch[1].trim()
  }
  
  // 如果以 ## 或 # 或 > 開頭，可能是直接的 markdown 內容
  const trimmedResponse = response.trim()
  if (trimmedResponse.match(/^[#>]/)) {
    return trimmedResponse
  }
  
  // 如果內容較長且包含markdown格式，嘗試移除思考過程後返回
