...
const showEdgeLabels = ref(true) // 默認顯示邊標籤
const expandedSelfLoops = ref(new Set()) // 展開的自環項
const showSimulationFinishedHint = ref(false) // 模擬結束後的提示
const wasSimulating = ref(false) // 追蹤之前是否在模擬中
...
const closeDetailPanel = () => {
  ...
  expandedSelfLoops.value = new Set() // 重置展開狀態
}
...
const renderGraph = () => {
  ...
  // 處理邊數據，計算同一對節點間的邊數量和索引
  const edgePairCount = {}
  const selfLoopEdges = {} // 按節點分組的自環邊
  ...
  // 記錄當前處理到每對節點的第幾條邊
  const edgePairIndex = {}
  const processedSelfLoopNodes = new Set() // 已處理的自環節點
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      ...
      edges.push({
        ...
        rawData: {
          ...
          selfLoopEdges: allSelfLoops // 存儲所有自環邊的詳細信息
        }
      })
      return
    }
    ...
    if (totalCount > 1) {
      // 均勻分佈曲率，確保明顯區分
      // 曲率範圍根據邊數量增加，邊越多曲率範圍越大
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2
      
      // 如果邊的方向與標準化方向相反，翻轉曲率
      // 這樣確保所有邊在同一參考系下分佈，不會因方向不同而重疊
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const getLinkPath = (d) => {
    ...
    // 計算曲線控制點 - 根據邊數量和距離動態調整
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // 垂直於連線方向的偏移，根據距離比例計算，保證曲線明顯可見
    // 邊越多，偏移量佔距離的比例越大
    const pairTotal = d.pairTotal || 1
    const offsetRatio = 0.25 + pairTotal * 0.05 // 基礎25%，每多一條邊增加5%
  ...
  const getLinkMidpoint = (d) => {
    ...
    // 二次貝塞爾曲線公式 B(t) = (1-t)²P0 + 2(1-t)tP1 + t²P2, t=0.5
