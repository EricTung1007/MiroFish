...
// 直接在初始化時從 query 參數獲取 maxRounds，確保子組件能立即獲取到值
const maxRounds = ref(route.query.maxRounds ? parseInt(route.query.maxRounds) : null)
const minutesPerRound = ref(30) // 默認每輪30分鐘
...
const handleNextStep = () => {
  // Step3Simulation 組件會直接處理報告生成和路由跳轉
  // 這個方法僅作為備用
...
const loadSimulationData = async () => {
  try {
    ...
    if (simRes.success && simRes.data) {
      ...
      // 獲取 simulation config 以獲取 minutes_per_round
      try {
...
const loadGraph = async (graphId) => {
  // 當正在模擬時，自動刷新不顯示全屏 loading，以免閃爍
  // 手動刷新或初始加載時顯示 loading
  if (!isSimulating.value) {
...
onMounted(() => {
  ...
  // 記錄 maxRounds 配置（值已在初始化時從 query 參數獲取）
  if (maxRounds.value) {
